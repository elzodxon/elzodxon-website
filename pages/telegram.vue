<template>
  <main class="min-h-screen">
    <AppHeader class="mb-8" title="Telegram Posts" :description="description" />
    ...
    <div v-else-if="error" class="text-center py-12">
      <p class="text-red-500 dark:text-red-400 mb-4">
        {{ error.message || "Failed to load Telegram posts" }}
      </p>
    ...
    <div v-else class="text-center py-12">
      ...
      <div class="flex justify-center gap-2">
        <UButton color="primary" variant="soft" @click="syncPosts(5)" :loading="syncing">
          Sync from Telegram
        </UButton>
        <UButton color="gray" variant="soft" @click="syncPosts(15)" :loading="syncing">
          Load more (15 pages)
        </UButton>
      </div>
    </div>
    ...
    <div class="fixed bottom-6 right-6 flex flex-col gap-2">
      <UTooltip text="Load more older posts (10 pages)">
        <UButton
          color="gray"
          variant="solid"
      ...
      <UTooltip text="Sync latest posts from Telegram">
        <UButton
          color="primary"
          variant="solid"
...
<script setup>
const description =
  "Latest updates and thoughts from my Telegram channel. Follow along for real-time insights and updates.";

useSeoMeta({
  title: "Telegram Posts | Elzodxon Sharofaddinov",
  description,
});
...
const { data, pending, error, refresh } = await useAsyncData(
  "telegram-posts",
  () => $fetch("/api/telegram/posts", { query: queryParams.value }),
  {
    watch: [currentPage],
  }
);
...
const syncPosts = async (pages = 5) => {
  syncing.value = true;
  try {
    const result = await $fetch("/api/telegram/sync", { 
      method: "POST",
      query: { pages }
    });
